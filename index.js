...
const cron = require("node-cron"); //run program terpisah

const sessionName = "kyoo";
const owner = ["6289502976892"];
const {
    default: kyowaConnect,
...
//Set Untuk AD REPLY
const url = "https://www.linkedin.com/in/heireev";
const thumbPath = fs.readFileSync(`./files/media/img1.jpg`);
const myAds = {
    externalAdReply: {
        title: "Kyowa-Bot",
        body: "Made by M. Rifqi Irawan",
        thumbnail: thumbPath,
        sourceUrl: url,
    },
};
...
async function startHisoka() {
    ...
    client.ev.on("messages.upsert", async (chatUpdate) => {
        ...
        try {
            ...
            require("./kyowa")(client, m, chatUpdate, store);
        } catch (err) {
            console.log(err);
        }
    });
    ...
    client.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            if (reason === DisconnectReason.badSession) {
                ...
                console.log(
                    `Device Logged Out, Please Delete Folder Session kyoo and Scan Again.`,
                );
                process.exit();
            ...
            client.sendMessage(owner + "@s.whatsapp.net", {
                text: `*Kyowa-Bot SMEKSA* telah aktif`,
            });
        }
    ...
    client.sendDoc = async (
        ...
        let buffer = fs.readFileSync(docPath); // Read the PDF file as buffer
        return await client.sendMessage(
            jid,
            {
                document: buffer,
    ...
    // Fungsi untuk memberikan reaksi emot
    client.sendEmoticon = async (jid, emoticon, quoted = "", options) => {
    ...
    // Menjalankan taskChecker setiap menit
    cron.schedule("* * * * *", () => {
        const { taskReminder, groupTaskReminder } = require("./handler/studentTasks");
        taskReminder(client, myAds);
        groupTaskReminder(client, myAds);
    });
